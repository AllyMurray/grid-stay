...
import { listMemberDateLeaderboard, listSiteMembers } from '~/lib/auth/members.server';
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const [members, pendingInvites, leaderboard] = await Promise.all([
    listSiteMembers(),
    listPendingMemberInvitesForUser(user.id),
    listMemberDateLeaderboard(),
  ]);

  return Response.json({ members, pendingInvites, leaderboard }, { headers });
}
